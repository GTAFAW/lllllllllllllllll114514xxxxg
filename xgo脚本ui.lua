--[[xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxggggggggggggggggoooooooooooooooooooooooooSirius©2024 Sirius保留所有权利.]]
--[[Sirius previon ovdo列表高优先级 - 隐形 -  godmode  - 所有脚本按钮和通用脚本 - 聊天垃圾邮件检测 - 自定义脚本提示 - 通过玩家列表 -  http.request支持Sirius智能HTTP拦截的HTTP.Request支持 - 对Roblox本身的性能改进适中优先 - 窥视动画，如GTA Serverhop，在天空中的高点，然后在其他玩家的头部 - 聊天间谍跟踪：遵循他们窃窃私语的基于原始消息 - 星光 -  Chatlogs  -  GTA Serverhop  - 反垃圾邮件（聊天）公式，基于文本长度，帽，表情符号等 - 减少任何形式的SIRIUS检测 - 在较低FPS上自动降低图形，确保没有误报潜在的未来设置选项 - 阻止整个域或只是Sirius智能流拦截的具体页面。 在逐个情况下执行此操作，例如，被阻止= {“link.com”，true}  - 真实是它是它的域或不 -  serverhop类型（默认/ gta） - 钩子特定函数，以减少外部脚本的需要]]
...
local siriusValues = {
	...
	games = {
		...
		TowerOfHell = {
			name = "地狱之塔",
			description = "一个很难受欢迎的parkouring游戏，具有随机水平和修饰符。 执行此脚本以获得不公平的优势.",
			id = 1962086868,
			enabled = true,
			raw = "TowerOfHell",
			minimumTier = "Free",
	...
	senseRaw = "https://raw.githubusercontent.com/GTAFAW/goto/refs/heads/main/libraryui.lua",
...
local siriusSettings = {
	{
		...
		categorySettings = {
			...
			{
				name = '现在玩通知',
				description = '活动时，天狼星将在您的音乐队列中的下一首歌播放时通知您.',
				settingType = 'Boolean',
				current = true,
			...
			{
				name = '基于客户的反踢',
				description = '取消涉及客户发送的任何踢的请求。 这可能会提示从Adonis或类似的反作用中检测。 您需要重新加入并重新运行Sirius以切换。',
				settingType = 'Boolean',
				current = false,
	...
	{
		name = '检测',
		description = 'Sirius发现并阻止任何恶意或可能对您的幸福有害.',
...
local function UpdateHome()
	...
	-- Home Title
	homeContainer.Title.Text = "欢迎 使用xgo, "..localPlayer.DisplayName
