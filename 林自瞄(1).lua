local uiName = "林";
local a = setmetatable({}, {__index = function(self, b)
...
function l:Tween(m, n, o, p, q)
    return a.TweenService:Create(n, TweenInfo.new(o or 0.25, Enum.EasingStyle[p or "Linear"], Enum.EasingDirection[q or "InOut"]), m)
end
...
function library:SelectNotify(Y)
    ...
    a1.True.MouseButton1Click:Connect(function()
        a0 = false
        a2 = true
        notifAmount = notifAmount - 1
    ...
    a1.False.MouseButton1Click:Connect(function()
        a0 = false
        a2 = true
        notifAmount = notifAmount - 1
        removedPos = a1.Position.Y.Offset
    ...
    task.spawn(function()
        ...
        while a0 and task.wait() do
            ...
            if notifAmount > Z then
                a1:TweenPosition(UDim2.new(1, -5, 1, a3.Y.Offset - 65 * (notifAmount - _)), Enum.EasingDirection.Out, Enum.EasingStyle.Quint, Y.TweenVerticalSpeed, true)
                Z = Z + 1
            end
            if notifAmount < Z then
        ...
        a1:TweenPosition(UDim2.new(1, 300, 1, a4.Y.Offset), Enum.EasingDirection.Out, Enum.EasingStyle.Quint, Y.TweenOutSpeed, true)
...
function library:Notify(Y)
    ...
    a1.Position = UDim2.new(1, 300, 1, -5)
    a1.Transparency = 0.05
    a1.InputBegan:Connect(function(a5)
...
function library:CreateTab(a7)
    ...
    ab.Name = "GlowCorner"
    ab.Parent = aa
    ac.Color = ColorSequence.new {--渐变颜色顺序
    ...
    function ag:NewBind(ai, av, aj)
        ...
        aD.Parent = aC
        a.UserInputService.InputBegan:Connect(function(aE, aF)
            if aF then
                return
            end
    ...
    function ag:Slider(ai, j, av, aJ, aK, aL, aj)
        ...
        aO.InputBegan:Connect(function(b0)
        ...
        a.UserInputService.InputEnded:Connect(function(b0)
            if aY and b0.UserInputType == Enum.UserInputType.MouseButton1 or b0.UserInputType == Enum.UserInputType.Touch then aY = false end
        end)
        a.UserInputService.InputChanged:Connect(function(b0)
            if aY == true then library.flags[j]:SetValue() end
        end)
        aS.Focused:Connect(function() aZ = true end)
        aS.FocusLost:Connect(function()
            aZ = false
            if aS.Text == "" then
                library.flags[j]:SetValue(av)
            end
        end)
        aS:GetPropertyChangedSignal("Text"):Connect(function()
            if not aZ then
                return
            end
    ...
    function ag:Dropdown(ai, j, b1, aj)
        ...
        b9.PaddingTop = c(0, 6)
        local ba = false
        b8:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        ...
        b4.Focused:Connect(function()
            if ba then return end bi()
        end)
    ...
    function ag:Textbox(ai, j, aj)
        ...
        aT.Name = "BoxCorner"
        aT.Parent = aS
        aS.FocusLost:Connect(function(bm)
            if not bm then
                return
            else
                aj(aS.Text)
...
setmetatable(getgenv().library, {__newindex = function(self, bn, bo)
    if bn == "Name" then
        x.Text = "   " .. bo
        return true
    end
...
local Aimbot = library:CreateTab("自瞄")

local FOVring = Drawing.new("Circle")
Aimbot:Toggle("自瞄圆圈显示", "v0", false, function(v0)
    FOVring.Visible = v0
end)
Aimbot:Slider("范围厚度", "v1", 3, 0, 100, false, function(v1)
    FOVring.Thickness = v1
end)
Aimbot:Slider("范围大小", "v2", 100, 0, 200, false, function(v2)
    FOVring.Radius = v2
end)
FOVring.Filled = false
FOVring.Color = Color3.fromRGB(255, 255, 255)
local v4
Aimbot:Toggle("自瞄开关", "v3", false, function(v3)
    if v3 then
        v4 = true
    else
        v4 = false
...
FOVring.CircleTransparency = 0.7
...
function getClosestPlayerToCursor(trg_part)
    local nearest = nil
    for i, v in pairs(PR:GetPlayers()) do
        if v ~= LP and LP.Character and v.Character and v.Character:FindFirstChild(trg_part) then
            if LP.Character:FindFirstChild(trg_part) then
                local ePos, vissss = workspace.CurrentCamera:WorldToViewportPoint(v.Character[trg_part].Position)
                local AccPos = Vector2.new(ePos.x, ePos.y)
                local mousePos = Vector2.new(workspace.CurrentCamera.ViewportSize.x / 2, workspace.CurrentCamera.ViewportSize.y / 2)
                local distance = (AccPos - mousePos).magnitude
                if vissss and distance <= MaxDistance then
                    nearest = v
                end
            end
        end
...
RS.RenderStepped:Connect(function()
    ...
    if v4 == true and closest and closest.Character:FindFirstChild("Head") then
